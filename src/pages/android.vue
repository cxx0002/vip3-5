<template>
  <div class="android">
    <div class="header">
      <p class="title">
        <md-icon name="arrow-left" style="float:left;font-weight:600" @click="back"></md-icon>会员中心
      </p>
    </div>
    <div class="header2">
    ...
    <div class="content">
      ...
      <div class="zffs">
        <div v-for="(item,key) in zfList" :key="key" @click="changezf(item.sel)">
          <img :src="item.img" alt style="float:left" />
          <span>{{item.name}}</span>
          <img src="@/assets/andorid/sel.png" alt v-show="sel==item.sel" class="zftp" />
          <img src="@/assets/andorid/def.png" alt v-show="sel!=item.sel" class="zftp" />
        </div>
      </div>
      ...
      axios({
        ...
        if (this.loginValid) {
          ...
          let data = {
            ...
            purchase_channel: this.sel,
            subscriptions: subscription,
            preferentialPrice: this.selTc.preferentialPrice,
            name: "购买" + this.selTc.name + "VIP"
          };
          this.$bridge.callhandlerAndroid(
            "td_js_function_purchase",
            data,
            function(responseData) {}
          );
        } else {
